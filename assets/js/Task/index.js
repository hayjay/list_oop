...
function buildTaskList(response) {
    ...
    if(!isEmpty(response)){
        ...
        for(var i = 0;i < total;i++ ){
            html +='<div class="row task-item" title="To Edit, Double Click The List">';
...
function addList() {
    ...
    if(!isEmpty($.trim(title))){
        var url = './list/add';
        LIST.ajaxSubmitDataCallback(url,{'title' : title},'json',function (response) {
            if(!isEmpty(response) && response.status == 'success'){
                init();
                $("#task_name").val('');
            }else{
...
function listAction(url,id) {
    if(!isEmpty(parseInt(id)) && !isEmpty(url)){
        LIST.ajaxSubmitDataCallback(url,{'id' : id},'json',function (response) {
            if(!isEmpty(response) && response.status == 'success'){
                init();
            }else{
                if(!isEmpty(response.message)){
...
function removeCompleted() {
    var url = './list/removeCompleted';
    LIST.ajaxSubmitDataCallback(url,{},'json',function (response) {
        if(!isEmpty(response) && response.status == 'success'){
            init();
        }else{
            if(!isEmpty(response.message)){
...
function editList(element) {
    if(!isEmpty(element)){
        ...
        if(!isEmpty(title) && !isEmpty(id)){
            var url = './list/edit';
            LIST.ajaxSubmitDataCallback(url,{'title' : title,'id' : id},'json',function (response) {
                if(!isEmpty(response) && response.status == 'success'){
                    init();
                }else{
                    if(!isEmpty(response.message)){
